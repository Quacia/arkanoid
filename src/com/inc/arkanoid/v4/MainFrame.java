...
public class MainFrame extends JFrame {
	...
	private void ballThread() {
		new Thread(() -> {
			...
			while (true) {
				if(paused) {
					JOptionPane.showMessageDialog(null, "계속 진행하시겠습니까?");
					paused = false;
				}
				collision();
		...
		new Thread(()-> {
			...
			if(x1 < ball.getP().x + ball.BALL_SIZE && ball.getP().x < x2 && y1 < ball.getP().y+ball.BALL_SIZE && ball.getP().y < y2) {
				...
				synchronized(Block.blockList) {
					...
					//int itemNum = 0;
					if(itemNum == 0)  {
						item = new Point(b.getP().x + Block.BLOCK_W/2, b.getP().y);
					}
	...
	private synchronized void itemThread(Point item) {
		new Thread(() -> {
